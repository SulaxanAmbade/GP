...
def hash_password(
    ...
    """Create a salted PBKDF2 password hash for secrets.toml."""
...
def verify_password(password, stored_password):
    """Verify a PBKDF2 hash, with temporary plaintext compatibility."""
    ...
    if not stored_password.startswith(
        ...
        # Backward compatibility for existing secrets. Replace these
        # plaintext values with hashes as soon as possible.
    ...
    try:
        ...
        # Prevent malformed configuration from causing excessive work.
...
def get_app_users():
    """
    Get configured username/password accounts.

    Recommended:
    .streamlit/secrets.toml

    [users]
    admin = "pbkdf2_sha256$600000$salt$hash"
    analyst = "pbkdf2_sha256$600000$salt$hash"

    Environment alternative:
    APP_USERS_JSON='{"admin":"password"}'

    ADMIN_PASSWORD remains supported as a legacy admin account.
    """
...
def admin_login():
    ...
    if not configured_users:
        st.error(
            "No user accounts are configured. Add a [users] section "
...
def parse_uploaded_file(uploaded_file):
    ...
    fixed_language_count = shifted_language_mask.sum()
...
def split_domain_basis(pattern):
    ...
    # Some source files wrap the complete pattern in '*', while
    # others store it as domain*basis. Support both layouts.
...
def normalize_domain(value):
    """Return a hostname-like value for reliable comparison."""
    ...
    hostname = parsed.hostname or value.split("/")[0]
...
def get_url_parts(url):
    """Return the normalized hostname and searchable URL content."""
    ...
    return (
        hostname,
        normalize_search_text(searchable_content)
    )


def domains_match(url_domain, pattern_domain):
    """Match the same domain and its subdomains."""
...
def make_basis_catalog(pattern_df):
    """Create a deduplicated global basis catalog for coverage reports."""
    ...
    for _, row in pattern_df.iterrows():
        ...
        # A coverage basis must contain both parts created by the
        # domain-basis split. Domain-only rows are intentionally ignored.
        if (
    ...
    # Coverage is global: the source domain is retained only for reference
    # and never limits which URL can match a basis.
    catalog = (
        catalog
        .groupby(
            [
...
def word_forms_match(first_word, second_word):
    """Match exact words plus common singular/plural URL variations."""
...
def basis_matches_url(normalized_basis, searchable_url):
    """Match basis words in sequence, tolerating singular/plural forms."""
    ...
    for start in range(
        ...
        if all(
            word_forms_match(basis_word, url_word)
            for basis_word, url_word in zip(
                basis_words,
                url_window
            )
        ):
...
def coerce_metric(series):
    """Convert formatted impression or revenue values to numbers."""
...
def read_coverage_csv(uploaded_file):
    """Read common CSV encodings and automatically detect delimiters."""
...
def find_suggested_column(columns, candidates):
    """Suggest a CSV column from normalized names."""
...
def remove_report_total_rows(input_df):
    """Remove summary labels that are not real URLs."""
    ...
    return (
        input_df.loc[~excluded_mask].copy(),
        int(excluded_mask.sum())
    )


def create_coverage_report(input_df, basis_catalog):
    """Map all matching bases to every URL and build report tables."""

    detail_rows = []
    url_rows = []
    selected_language = str(
        basis_catalog["language_code"].iloc[0]
    )

    # A CSV may contain several keyword rows for the same URL. Roll
    # those rows up first so URL counts and totals are not duplicated.
    input_df, _ = remove_report_total_rows(input_df)
    input_df = input_df.copy()
    input_df["URL"] = input_df["URL"].astype(str).str.strip()
    input_df = (
        input_df
        .groupby(
            "URL",
    ...
    for _, input_row in input_df.iterrows():
        ...
        url = str(input_row["URL"]).strip()
        impressions = input_row["Keyword Impressions"]
        revenue = input_row["Revenue"]
        url_domain, searchable_url = get_url_parts(url)

        matches = basis_catalog[
            basis_catalog["normalized_basis"].apply(
                lambda basis: basis_matches_url(
                    basis,
                    searchable_url
                )
            )
        ...
        for _, match in matches.iterrows():
            matched_bases.append(str(match["basis"]))
        ...
        url_rows.append(
            {
                ...
                "Matching Bases": " | ".join(matched_bases),
                "Keyword Impressions": impressions,
                "Revenue": revenue
            }
        )
    ...
    url_summary_df = pd.DataFrame(url_rows)
    ...
    coverage_pivot = (
        ...
        .agg(
            URLs=("URL", "count"),
            Keyword_Impressions=("Keyword Impressions", "sum"),
            Revenue=("Revenue", "sum")
        )
        .rename(
            columns={
    ...
    covered_detail = detail_df[
        detail_df["Coverage Status"] == "Covered"
    ]
...
def render_coverage_report(final_df):
    """Render the complete sidebar coverage-report workflow."""
    ...
    st.write(
        ...
        "A basis matches when its normalized words appear together in "
        "the URL path, including common singular/plural variations."
    )
    ...
    available_languages = sorted(
        complete_basis_catalog["language_code"]
        .dropna()
        .astype(str)
        .str.strip()
    ...
    if not available_languages:
        st.warning(
            "⚠️ No language codes are available in the active dataset."
        )
        return
    ...
    selected_language = st.selectbox(
        ...
        help=(
            "Only bases with this language code will be checked "
            "against the submitted URLs."
        )
    )
    ...
    st.caption(
        f"{len(basis_catalog):,} unique bases are available for "
                ...
                if source_df.empty:
                    ...
                    with mapping2:
                        impressions_column = st.selectbox(
                            ...
                            index=(
                                ...
                                if suggested_impressions in columns
                                else 0
                            ),
                    ...
                    with mapping3:
                        revenue_column = st.selectbox(
                            ...
                            optional_columns,
                            index=(
                                optional_columns.index(suggested_revenue)
                                if suggested_revenue in columns
                                else 0
                            ),
                    ...
                    prepared_input = pd.DataFrame(
                        {
                            "URL": source_df[url_column]
                        }
                    )

                    prepared_input["Keyword Impressions"] = (
                        coerce_metric(source_df[impressions_column])
                        if impressions_column != "None"
                        else 0
                    )

                    prepared_input["Revenue"] = (
                        coerce_metric(source_df[revenue_column])
    ...
    if st.button(
        ...
        with st.spinner("Checking URL coverage..."):
            ...
            st.session_state["coverage_report_language"] = (
                selected_language
            )
    ...
    if (
        st.session_state.get("coverage_report_language")
        != selected_language
    ):
        report = None
    ...
    covered_urls = int(
        (url_summary_df["Coverage Status"] == "Covered").sum()
    )
    uncovered_urls = total_urls - covered_urls
    coverage_rate = (
        covered_urls / total_urls * 100
        if total_urls
        else 0
    )
    ...
    with basis_tab:
        ...
        st.caption(
            "When one URL matches multiple bases, its performance is "
            "attributed to each matching basis. Use URL Summary for "
    ...
    download1.download_button(
        ...
        data=url_summary_df.to_csv(index=False),
    ...
    download2.download_button(
        ...
        data=basis_pivot.to_csv(index=False),
...
with st.sidebar:
    ...
    if st.session_state.get(
        ...
        st.success(
            "🟢 Signed in as "
            f"{st.session_state.get('authenticated_username', 'admin')}"
        )
        ...
        with st.expander(
            ...
            with st.form(
                ...
                generate_hash = st.form_submit_button(
                    "Generate Hash",
                    use_container_width=True
                )
            ...
            generated_hash = st.session_state.get(
                "generated_password_hash"
            )
            ...
            if generated_hash:
                ...
                st.caption(
                    "Copy this value into the [users] section "
...
if selected_page == "Coverage Report":

    render_coverage_report(final_df)
...
if search_string.strip():
    ...
    if final_df.empty:
        ...
        # Accept both spaces and commas as separators between
        # multiple search terms. This allows inputs such as:
        # 1341291255 1341291256 1341270974
        # or:
        # labor day, hair shampoo, 1341291255
        search_terms = [
            term.strip()
        ...
        # Store which search terms matched each row
        matched_terms = {
            index: []
        ...
        for search_term in search_terms:
            ...
            # -------------------------------------------------
            # NORMALIZE SEARCH TERM FOR URL PATTERN
            # -------------------------------------------------
            ...
            # Add this term's results to overall results
        ...
        if not search_results.empty:
            ...
            st.warning(
                f"❌ No URL patterns or URL pattern IDs "
                f"found for the entered search terms."
            )
# =============================================================
# FULL DATASET
# =============================================================
